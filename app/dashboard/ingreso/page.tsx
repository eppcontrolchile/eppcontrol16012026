...
export default function IngresoPage() {
  ...
  const formatCatalogoLabel = (row: CatalogoEppRow) => {
    ...
    const stockTxt =
      Number.isFinite(Number(row.stock_actual)) && Number(row.stock_actual) > 0
        ? ` | Stock actual: ${Number(row.stock_actual).toLocaleString("es-CL")}`
        : "";

    return `${extra ? `${base} | ${extra}` : base}${stockTxt}`;
  };
      ...
      try {
        ...
        const rows: CatalogoEppRow[] = Array.isArray(raw?.rows)
          ? raw.rows.map((r: any) => ({
              ...
              talla: r?.talla ?? null,
              stock_actual:
                r?.stock_actual == null || r?.stock_actual === ""
                  ? null
    ...
    const fetchHistorial = async () => {
      try {
        ...
        const data: IngresoHistorialRow[] = arr.map((r: any) => {
          ...
          const total =
            Number.isFinite(cantidad) && Number.isFinite(valorUnitario)
  ...
  const totalPaginas = Math.ceil((historialFiltrado.length || 0) / ITEMS_POR_HOJA);
      ...
      try {
        ...
        const itemsToSend = items.map((item) => {
          if (!item.valorUnitario || !item.tipoIVA) {
            throw new Error("Debes ingresar monto unitario y tipo de IVA");
          }
          ...
          return {
            producto_id:
              item.modoIngreso === "existente" ? (item.productoId || null) : null,
            categoria:
              item.categoria === "Otro"
  ...
  const handleOrden = (campo: keyof IngresoHistorialRow) => {
    if (ordenCampo === campo) {
  ...
  const modificarIngreso = async (row: IngresoHistorialRow) => {
    ...
    const nuevaCantidad = prompt(
      "Cantidad (solo permitido si el lote no tiene consumo):",
      String(row.cantidad)
    );
  ...
  return (
    <div className="w-full space-y-6">
            ...
            <div key={index} className="rounded border p-3 space-y-2">
              <div className="rounded border bg-zinc-50 p-3 space-y-2">
                ...
                {item.modoIngreso === "existente" && (
                  <div className="space-y-2">
                    ...
                    >
                      <option value="">
                        {catalogoLoading ? "Cargando catálogo..." : "Selecciona un EPP existente"}
                      </option>
                      {catalogo.map((row) => (
              ...
              {item.categoria === "Otro" && (
                <input
                  ...
                  onChange={(e) => updateItem(index, "categoriaOtro", e.target.value)}
                  disabled={item.modoIngreso === "existente"}
                />
              )}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  updateItem(index, "cantidad", raw === "" ? "" : Number(raw));
                }}
                className="input"
              ...
              <select
                value={item.tipoIVA}
                onChange={(e) => updateItem(index, "tipoIVA", e.target.value as any)}
                className="input"
              >
      ...
      <h2 className="text-xl font-semibold">Historial de ingresos de EPP</h2>
        ...
        <table className="w-full min-w-full border-collapse border border-slate-300 text-sm">
          <thead>
            <tr>
              ...
              >
                Valor unitario ($ IVA incl.){" "}
                {ordenCampo === "valorUnitario" && (ordenDireccion === "asc" ? "▲" : "▼")}
              </th>
              <th
          ...
          <tbody>
            ...
            {historialPaginado.map((row, idx) => (
              ...
              >
                <td className="p-2 border border-slate-300">{formatFechaCL(row.fecha)}</td>
                <td className="p-2 border border-slate-300">
                  <div className="font-medium">{formatDocCompra(row.compra)}</div>
                  {row.compra?.fecha_documento ? (
                    <div className="text-xs text-zinc-500">
                      Doc: {formatFechaCL(row.compra.fecha_documento)}
                    </div>
                  ) : null}
                </td>
                <td className="p-2 border border-slate-300">
                  <div
                    ...
                    title={formatProveedorCompra(row.compra)}
                  >
                    {formatProveedorCompra(row.compra)}
                  </div>
                </td>
                <td className="p-2 border border-slate-300">{row.categoria}</td>
                <td className="p-2 border border-slate-300">
                ...
                <td className="p-2 border border-slate-300">{row.talla ?? "No aplica"}</td>
                <td className="p-2 border border-slate-300 text-right">{row.cantidad}</td>
